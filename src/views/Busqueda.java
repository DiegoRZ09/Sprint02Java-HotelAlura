...
public class Busqueda extends JFrame {
	...
	private void llenarTablaHuespedes() {
		...
		try {
			for(Huesped huespedesL: huespedes) {
				modeloH.addRow(new Object[] {huespedesL.getId(), huespedesL.getNombre(), huespedesL.getApellido(), huespedesL.getFechaNac(), huespedesL.getNacionalidad(), huespedesL.getTelefono(), huespedesL.getIdReserva()});
			}
		} catch (Exception e) {
			throw new RuntimeException(e);
		}
	...
	private void llenarTablaHuespedesPorIdReserva() {
		...
		try {
			for(Huesped huespedes: huespedPorId) {
				modeloH.addRow(new Object[] {huespedes.getId(), huespedes.getNombre(), huespedes.getApellido(), huespedes.getFechaNac(), huespedes.getNacionalidad(), huespedes.getTelefono(), huespedes.getIdReserva()});
			}
		} catch(Exception e){
			throw new RuntimeException(e);
		}
	...
	private String calcularValorActualizado(Date fechaE, Date fechaS) {
		
		LocalDate entrada = LocalDate.parse((CharSequence) fechaE, DateTimeFormatter.ISO_LOCAL_DATE);
		LocalDate salida = LocalDate.parse((CharSequence) fechaS, DateTimeFormatter.ISO_LOCAL_DATE);
		
		int dias = (int) ChronoUnit.DAYS.between(entrada, salida);
		double valorPorDia = 800;
		
		double valordelaReserva = dias*valorPorDia;
		
		String valorenString = String.valueOf(valordelaReserva);
		
		System.out.println(valorenString);
